...
def val(model, dataloader, post_process, epoch, device, save_dir, args):
...
def train_one_epoch(model, train_dataloader, val_dataloader, 
                    optimizer, post_process, device, i_epoch, icv, args):
  
  save_iters = 1000

  save_dir = os.path.join(args.output_dir, "cv{}".format(icv))
  if not os.path.exists(save_dir):
    print("Directory {} doesn't exist, create a new.".format(save_dir))
    os.makedirs(save_dir)
  post_process.set_output_dir(save_dir)
  ...
  if args.do_eval:
    val(model, val_dataloader, post_process, i_epoch, device, save_dir, args)
...
def main():
  ...
  train_data_dirs = ['data/cv5_data/cv{}/train/'.format(i) for i in range(5)]
  test_data_dirs = ['data/cv5_data/cv{}/test/'.format(i) for i in range(5)]

  for icv in range(5):
    train_dataset = PriDataset(args, train_data_dirs[icv], args.train_batch_size)
    ...
    val_dataset = PriDataset(args, test_data_dirs[icv], args.eval_batch_size)
    ...
    for i_epoch in range(int(start_epoch), int(start_epoch + args.num_train_epochs)):
      ...
      train_one_epoch(model, train_dataloader, val_dataloader, optimizer, post_process, device, i_epoch, icv, args)
