...
class ESM2(nn.Module):
    def __init__(
        ...
        self.alphabet = alphabet # alphabet is similar to dictionary
        self.alphabet_size = len(alphabet) # size of dictionary
    ...
    def forward(self, tokens, repr_layers=[], need_head_weights=False, return_contacts=False):
        ...
        if self.token_dropout:
            ...
            src_lengths = (~padding_mask).sum(-1) # valid lengths of each item in batch
            mask_ratio_observed = (tokens == self.mask_idx).sum(-1).to(x.dtype) / src_lengths
            x = x * (1 - mask_ratio_train) / (1 - mask_ratio_observed)[:, None, None] # mul ratio(i) to x(i), i in batch 
